...
class RobotApp:
    ...
    def _build_ui(self):
        ...
        self._build_left_column(body); self._build_right_panel(body)
        self.thruster_panel = ThrusterPanel(outer, labels=THRUSTER_LABELS, roles=THRUSTER_ROLES); self.thruster_panel.pack(fill=tk.X, pady=(8, 0))
    ...
    def _build_left_column(self, parent):
        ...
        for i, (lbl_txt, key) in enumerate(items):
            ...
            self.telemetry_labels[key] = tk.Label(cell, text="—", font=self.fnt_mono_sm, bg=THEME["panel"], fg=THEME["cyan"]); self.telemetry_labels[key].pack()
    ...
    def _build_right_panel(self, parent):
        ...
        canvas.bind("<Configure>", lambda e: canvas.itemconfig(win_id, width=e.width)); canvas.configure(yscrollcommand=scrollbar.set)
        ...
        self.btn_arm = GlowButton(ctrl, "ARM VEHICLE", THEME["text_dim"], self.toggle_arm, icon="▲", disabled=True); self.btn_arm.pack(fill=tk.X, pady=2)
        SeparatorLine(ctrl, label=" OPERATIONS").pack(fill=tk.X, pady=(4, 1))
        self.btn_deploy = GlowButton(ctrl, "DEPLOY", THEME["text_dim"], self.deploy_mission, icon=" ⬇ ", disabled=True); self.btn_deploy.pack(fill=tk.X, pady=2)
        self.btn_retrieve = GlowButton(ctrl, "RETRIEVE", THEME["text_dim"], self.retrieve_mission, icon=" ⬆ ", disabled=True); self.btn_retrieve.pack(fill=tk.X, pady=2)
    ...
    def _section_header(self, parent, title, color):
        ...
        tk.Frame(bar, bg=color, width=3, height=14).pack(side=tk.LEFT); tk.Label(bar, text=f"  {title}", font=self.fnt_sub, bg=THEME["panel"], fg=color).pack(side=tk.LEFT)
    ...
    def _sync_armed_state(self, is_armed: bool):
        ...
        self.armed_state = is_armed; self.thruster_panel.set_armed(is_armed)
        self.btn_arm.update_label("DISARM VEHICLE" if is_armed else "ARM VEHICLE", THEME["danger"] if is_armed else THEME["warning"])
