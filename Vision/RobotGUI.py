...
class TelemetryCard(tk.Frame):
    def __init__(self, parent, label, value, unit="", color=None, **kw):
        ...
        tk.Label(self, text=label, font=("Courier New", 7, "bold"),
                 bg=THEME["card"], fg=THEME["text_dim"],
        ...
        tk.Label(self, text=unit, font=("Courier New", 7),
                 bg=THEME["card"], fg=THEME["text_dim"],
...
class ThrusterBar(tk.Frame):
    ...
    def __init__(self, parent, label, role, index, **kw):
        ...
        badge_row.pack(fill=tk.X, padx=4, pady=(4, 1))

        tk.Label(badge_row, text=f"#{index+1}", font=("Courier New", 7, "bold"),
                 bg=THEME["card"], fg=THEME["text_dim"]).pack(side=tk.LEFT)

        tk.Label(badge_row, text=label, font=("Courier New", 8, "bold"),
                 bg=THEME["card"], fg=THEME["accent"]).pack(side=tk.RIGHT)

        self._canvas = tk.Canvas(self, bg=THEME["log_bg"],
                                 highlightthickness=1,
                                 highlightbackground=THEME["border"],
                                 width=44, height=140)
        self._canvas.pack(padx=4, pady=2)

        tk.Label(self, text=role, font=("Courier New", 6),
                 bg=THEME["card"], fg=THEME["text_dim"]).pack(pady=(0, 1))
    ...
    def _redraw(self):
        ...
        for frac in (0.25, 0.5, 0.75):
            y_top = int(mid * frac)
            y_bot = H - y_top
            c.create_line(0, y_top, W, y_top, fill=THEME["border"], dash=(2, 4))
            c.create_line(0, y_bot, W, y_bot, fill=THEME["border"], dash=(2, 4))

        if v >= 0:
            fc = self._gradient_color(v, self._POS_COLORS)
            ...
            fc = self._gradient_color(abs(v), self._NEG_COLORS)
        ...
        for y_off in (0, mid // 2, mid):
            ...
            c.create_line(W - 6, H - y_off - 2, W, H - y_off - 2,
                          fill=THEME["text_dim"], width=1)
    ...
    def _gradient_color(v, stops):
        ...
        lo = int(s)
        hi = min(lo + 1, n)
        return ThrusterBar._lerp_hex(stops[lo], stops[hi], s - lo)
...
class ThrusterPanel(tk.Frame):
    def __init__(self, parent, labels, roles, **kw):
        ...
        tk.Frame(tb, bg=THEME["success"], width=3).pack(side=tk.LEFT, fill=tk.Y)
        tk.Label(tb, text="  THRUSTER OUTPUT", font=("Courier New", 9, "bold"),
                 bg=THEME["panel"], fg=THEME["success"]).pack(side=tk.LEFT, pady=2)

        self._armed_badge = tk.Label(tb, text="  ◼ DISARMED  ",
                                     font=("Courier New", 7, "bold"),
                                     bg=THEME["panel"], fg=THEME["text_dim"])
        self._armed_badge.pack(side=tk.RIGHT, padx=8)

        self._peak_lbl = tk.Label(tb, text="PEAK: —%",
                                  font=("Courier New", 7, "bold"),
                                  bg=THEME["panel"], fg=THEME["text_dim"])
        self._peak_lbl.pack(side=tk.RIGHT, padx=12)

        tk.Label(tb, text="W/S=Fwd  A/D=Strafe  Q/E=Yaw  R/F=Depth  SPACE=Stop",
                 font=("Courier New", 7), bg=THEME["panel"],
                 fg=THEME["text_dim"]).pack(side=tk.LEFT, padx=16)
        ...
        for i, (lbl, role) in enumerate(zip(labels, roles)):
            if i > 0:
                tk.Frame(bar_row, bg=THEME["border"],
...
class RobotApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self._keys = {k: False for k in ('w', 's', 'a', 'd', 'q', 'e', 'r', 'f')}

        self.video_backend     = RobotLogic(self.log)
        self.telemetry_backend = TelemetryHandler(self.update_telemetry_display,
                                                  self.log)
    ...
    def _build_ui(self):
        ...
        # Body: LEFT = fixed camera + HUD  |  RIGHT = scrollable controls
    ...
    def _create_left_column(self, parent):
        """Fixed width = CAM_W + 2px border. Does NOT expand."""
        ...
        # ── Camera title bar ─────────────────────────────────────────────────
        title_bar = tk.Frame(left, bg=THEME["panel"], height=26)
        title_bar.pack(fill=tk.X)
        title_bar.pack_propagate(False)

        tk.Frame(title_bar, bg=THEME["accent"],
                 width=3).pack(side=tk.LEFT, fill=tk.Y)
        tk.Label(title_bar, text=f"  PRIMARY CAMERA FEED  [{CAM_W}×{CAM_H}]",
                 font=self.fnt_sub, bg=THEME["panel"],
                 fg=THEME["accent"]).pack(side=tk.LEFT, pady=3)

        self.ai_status_lbl = tk.Label(title_bar, text="AI: LOADING",
                                      font=self.fnt_lbl, bg=THEME["panel"],
                                      fg=THEME["warning"])
        self.ai_status_lbl.pack(side=tk.RIGHT, padx=8)

        self.cam_status_lbl = tk.Label(title_bar, text="● NO SIGNAL",
                                       font=self.fnt_lbl, bg=THEME["panel"],
                                       fg=THEME["danger"])
    ...
    def _create_telemetry_section(self, parent):
        ...
        card_defs = [
            ("BATTERY", "—", "V", THEME["warning"], "BATTERY"),
            ("CURRENT", "—", "A", THEME["accent"],  "CURRENT"),
            ("DEPTH",   "—", "m", THEME["cyan"],    "DEPTH"),
            ("HEADING", "—", "°", THEME["cyan"],    "HEADING"),
            ("ROLL",    "—", "°", THEME["purple"],  "ROLL"),
            ("PITCH",   "—", "°", THEME["purple"],  "PITCH"),
        ]
        for idx, (lbl, val, unit, col, key) in enumerate(card_defs):
            r, c = divmod(idx, 2)
            card = TelemetryCard(grid, lbl, val, unit, col)
    ...
    def _create_thruster_panel(self, parent):
        self.thruster_panel = ThrusterPanel(parent,
    ...
    def _bind_keys(self):
        for key in ('w', 's', 'a', 'd', 'q', 'e', 'r', 'f'):
            ...
            self.root.bind(f'<KeyRelease-{key.upper()}>',
                           lambda e, k=key: self._key_up(k))
    ...
    def _process_keys(self):
        if self.armed_state and self.telemetry_backend.running:
            S   = CONTROL_SPEED
            fwd = S  if self._keys['w'] else (-S if self._keys['s'] else 0.0)
            lat = S  if self._keys['d'] else (-S if self._keys['a'] else 0.0)
            yaw = S  if self._keys['e'] else (-S if self._keys['q'] else 0.0)
            thr = S  if self._keys['r'] else (-S if self._keys['f'] else 0.0)

            if any(self._keys.values()):
                self.telemetry_backend.set_motion(
                    forward=fwd, lateral=lat, throttle=thr, yaw=yaw)
        ...
        def _apply():
            ...
            if key in hud_map:
                ...
                if hk in self.telemetry_labels:
                    self.telemetry_labels[hk].config(text=str(value))
            ...
            if key == "STATUS" and "MODE" in self.telemetry_labels:
                self.telemetry_labels["MODE"].config(text=str(value),
                                                     fg=THEME["accent"])

            # ── Armed state sync ──────────────────────────────────────────────
            if key == "ARMED_STATE":
                is_armed = (str(value) == "⚠ ARMED")
                if is_armed != self.armed_state:
                    self._sync_armed_state(is_armed)
    ...
    def _poll_telemetry(self):
        ...
        self.root.after(TELEMETRY_POLL_MS, self._poll_telemetry)

    # =========================================================================
    #  VIDEO LOOP — renders at fixed CAM_W × CAM_H
    # =========================================================================
    ...
    def _update_video_loop(self):
        ...
        if frame is not None:
            ...
            self._current_frame = img_tk  # Hold reference — prevent GC
    ...
    def _set_system(self, name, status, color):
        ...
        if bk in self.system_labels:
            self.system_labels[bk].config(bg=color)
